...
class carga(vehiculos):
    ...
    def tojson(self):
        d=dict(tipo=__class__.__name__,atributos=dict(marca=self._vehiculos__marca,modelo=self._vehiculos__modelo,patente=self._vehiculos__patente,importebasico=self._vehiculos__impor_bas,cantidadkm=self._vehiculos__cant_km,pesocarga=self.__peso_carga))
        return d
    def __str__(self):
