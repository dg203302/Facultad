...
def funcionesdespachante():
    return render_template('funcionesdespachante.html', sucursal=db.session.query(Sucursal).filter(Sucursal.getid()==session['sucursal_seleccionada']).first()) #ineficiente
...
def registrarrecepcion():
    if request.method=='POST':
        ...
        nuevopaquete=Paquete(numeroenvio=numerodeenvio,peso=peso,nomdestinatario=nombre,dirdestinatario=direccion,observaciones='',idsucursal=sucu.getid())
        db.session.add(nuevopaquete)
        db.session.commit()
        return render_template('registrocorrecto.html', paquete=nuevopaquete)
    else:
        return render_template('registrarrecepcionpaquete.html', sucu=db.session.query(Sucursal).filter(Sucursal.getid()==session['sucursal_seleccionada']).first())
...
def registrarsalida():
    if request.method=='POST':
        ...
        while i<len(paquetes):
            if paquetes[i].getestado()!=False or paquetes[i].getidtransporte()!=None:
                i+=1
            else:
                break
        if i==len(paquetes):
            ...
            return render_template('registrarsalidatranspo.html', sucursalesdest=db.session.query(Sucursal).filter(Sucursal.getid()!=session['sucursal_seleccionada']).all())
...
def seleccionarpaquete():
    if request.method=='POST':
        ...
        for ids in session['paquetes_seleccionados']:
            paq=db.session.query(Paquete).filter(Paquete.getid()==ids).first()
            if paq:
        ...
        return render_template('seleccionarpaquete.html', paquetes=db.session.query(Paquete).filter(Paquete.getestado()==False and Paquete.getidrepartidor()==None))
...
def registrarllegada():
    if request.method=='POST':
        ...
        transporte_lleg=db.session.query(Transporte).filter(Transporte.getid()==session['transporte_llegado']).first()
        transporte_lleg.actfechallegada(datetime.datetime.now())
        paquetes=transporte_lleg.getpaquetes()
        for paquete in paquetes:
            paquete.entregado=True
        db.session.commit()
        return render_template('exitoregistrollegada.html', transp=db.session.query(Transporte).filter(Transporte.getid()==transporte_lleg.getid()).first())
    else:
        #en caso de que no hayan transportes muestra una pagina aparte
        transports=db.session.query(Transporte).filter(Transporte.getidsucursal()==session['sucursal_seleccionada']).all()
        i=0
        while i<len(transports):
            if transports[i].getfechahorallegada()!=None:
                i+=1
            else:
                break
        if len(transports)==i:
            return render_template('sintransportesdisponibles.html', sucu=db.session.query(Sucursal).filter(Sucursal.getid()==session['sucursal_seleccionada']).first())
        else:
            return render_template('registrodellegada.html', transportes=db.session.query(Transporte).filter(Transporte.getidsucursal()==session['sucursal_seleccionada']).all())
