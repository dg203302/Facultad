...
class arbol:
    ...
    def __init__(self,raiz=None):
        self.__raiz=raiz
#----------------------------------------------------------------------------------------#
    def get_grado(self,nodo_actual):
        if not nodo_actual:
            return 0
        return nodo_actual.get_grado()
#----------------------IMPORTANTE BALANCEO------------------------------------------------------------------#
    def balanceo(self,nodo_actual):
        if not nodo_actual:
            return 0
        return self.get_grado(nodo_actual.get_izq())-self.get_grado(nodo_actual.get_der())
    def rotar_derecha(self, y):
        x=y.get_izq()
        temp = x.get_der()
        x.set_der(y)
        y.set_izq(temp)
        y.set_grado(1 + max(self.get_grado(y.get_izq()), self.get_grado(y.get_der())))
        x.set_grado(1 + max(self.get_grado(x.get_izq()), self.get_grado(x.get_der())))
        return x
...
#-------------------------------------------------------------------------------------------------#
    def insertar(self,dato):
        if self.__raiz is None:
            ...
            self.insertar_recursivo_balanceado(self.__raiz,dato,0)
    def insertar_recursivo_balanceado(self, nodo_actual, dato, grado):
        if dato > nodo_actual.get_dato():
            if nodo_actual.get_der() is None:
                nodo_actual.set_der(nodo(dato, grado + 1))
            else:
                self.insertar_recursivo_balanceado(nodo_actual.get_der(), dato, grado + 1)
        elif dato < nodo_actual.get_dato():
            if nodo_actual.get_izq() is None:
                nodo_actual.set_izq(nodo(dato, grado + 1))
            else:
                self.insertar_recursivo_balanceado(nodo_actual.get_izq(), dato, grado + 1)
        elif dato == nodo_actual.get_dato():
            print('ya esta ingresado el nodo!')
            raise ValueError
        balance = self.balanceo(self.__raiz)
        if balance > 1 and valor < self.__raiz.get_izq().get_dato():
            return self.rotar_derecha(self.__raiz)
        if balance > 1 and valor > self.__raiz.get_izq().get_dato():
            self.set_izq(self.rotar_izquierda(self.__raiz.get_izq()))
            return self.rotar_derecha(self.__raiz)
        if balance < -1 and valor > self.__raiz.get_der().get_dato():
            return self.rotar_izquierda(self.__raiz)
        if balance < -1 and valor < self.__raiz.get_der().get_dato():
            self.__raiz.set_der(self.rotar_derecha(self.__raiz.get_der()))
            return self.rotar_izquierda(self.__raiz)
        return self.__raiz
#-----------------------------------------------------------------------------------------#
